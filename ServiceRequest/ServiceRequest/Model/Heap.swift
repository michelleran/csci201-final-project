...
public struct Heap<T> {
  ...
  private mutating func configureHeap(from array: [T]) {
    ...
    for i in stride(from: (nodes.count/2-1), through: 0, by: -1) {
        shiftDown(from: i, upTo: nodes.count)
    }
  }
  ...
  @discardableResult public mutating func remove() -> T? {
    ...
    if nodes.count == 1 {
      ...
      nodes[0] = nodes.removeLast()
        shiftDown(from: 0, upTo: nodes.count)
      return value
    }
  }
  ...
  internal mutating func shiftDown(from: Int, upTo: Int) {
    shiftDown(from: from, until: upTo)
  }
    
  public mutating func sorted() -> [T] {
    for index in nodes.indices.reversed() {
      nodes.swapAt(0, index)
      shiftDown(from: 0, upTo: index)
    }
    return nodes
  }
}
